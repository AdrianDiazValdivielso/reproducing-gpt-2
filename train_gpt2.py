...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        assert config.n_embd % config.n_head == 0 # input space dimensionality must be divisible by the amount of heads
        ...
        # not really a 'bias', more of a causal mask (tril means lower triangle). Named 'bias' to follow the OpenAI convention
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.ln_1 = nn.LayerNorm(config.n_embd) #First layer normalization (before Causal Self Attention)
        self.attn = CausalSelfAttention(config) #Allows looking at previous tokens to gather context
        self.ln_2 = nn.LayerNorm(config.n_embd) #Second layer normalization (before multilayer perceptron)
        self.mlp = MLP(config) #Multilayer perceptron
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            wpe = nn.Embedding(config.block_size, config.n_embd), #weight position encoding
    ...
    def forward(self, idx, targets=None): 
        ...
        x = pos_emb + tok_emb # pos_emb is the same for any input tokens, so it is broadcast to all input batches
        # forward the blocks of the transformer
        for block in self.transformer.h:
            x = block(x)
        # forward the final layernorm and the clasifier
...
class DataLoaderLite():
    def __init__(self, B, T):
        ...
        # at init load tokens from dict and store them in memory
...
max_length = 30
tokens = enc.encode("Hello, I'm a language model,") # the gpt2 tokenizer divides it in eight tokens
...
while x.size(1) < max_length:
    ...
    probs = F.softmax(logits, dim=-1) # F stands for torch.nn.functional, they don't internally store weights nor biases
    ...
    x = torch.cat((x, xcol), dim=1) # x now is (B, T+1)
